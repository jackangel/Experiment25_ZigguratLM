...
# ... (ModelConfig, RotaryPositionalEmbeddings, ChunkedMultiHeadCardPassingLayer, VQ layers are all identical) ...
@dataclass
class ModelConfig:
...
class DiscretizedManifoldTransformer(nn.Module):
    # ... (identical with scaled PE) ...
    def __init__(self, config: ModelConfig):
        super().__init__()
        self.config = config; self.wte = nn.Embedding(config.vocab_size, config.n_embd)
        self.rope = RotaryPositionalEmbeddings(config.n_embd, config.block_size)
        self.drop = nn.Dropout(config.dropout); self.stages, self.fusion_blocks = nn.ModuleList(), nn.ModuleList()
        ...
        self.ln_f = nn.LayerNorm(config.n_embd); self.lm_head = nn.Linear(config.n_embd, config.vocab_size, bias=False)
    ...
    def forward(self, idx, targets=None, return_indices=False):
        base_emb = self.wte(idx); abstraction_logits = self.controller(base_emb)
        ...
        tok_emb = (stacked_embeddings * weights).sum(dim=0); x = self.drop(tok_emb + self.rope(tok_emb))
        all_stage_indices, output_from_stage1, total_q_loss, total_e_loss = [], None, 0.0, 0.0
        for i, stage in enumerate(self.stages):
            stage_indices = []
            for block in stage:
                if return_indices: x, q_loss, e_loss, indices = block(x, return_indices=True); stage_indices.append(indices)
            ...
            if i < self.config.n_stages - 1:
                if x.shape[1] > 1: x = self.scaled_pe(self.fusion_blocks[i](x), scale=2**(i + 1))
                else: break
        final_hidden_state = self.ln_f(output_from_stage1); logits = self.lm_head(final_hidden_state)
        if return_indices: return logits, all_stage_indices
        if targets is not None:
            ce_loss = F.cross_entropy(logits.view(-1, logits.size(-1)), targets.view(-1), ignore_index=-1)
            total_layers = self.config.n_stages * self.config.n_layer_per_stage; conf_loss = torch.tensor(0.0, device=logits.device)
            if self.config.use_confidence_circuit and hasattr(self, 'confidence_head'):
                ...
                with torch.no_grad(): predicted_tokens = logits.argmax(dim=-1); correctness_targets = (predicted_tokens == targets).float()
...
# --- MODIFIED: Main training block with new features ---
if __name__ == '__main__':
    ...
    # --- Configuration ---
    BATCH_SIZE, LEARNING_RATE = 4, 3e-4
    EVAL_EVERY = 5000
    ...
    if latest_checkpoint:
        ...
        while True:
            ...
            if choice in ['c', 'continue']:
                ...
                print(f"Will continue training from {os.path.basename(resume_from_checkpoint)}.")
                break
            elif choice in ['chat']:
                ...
                print("Invalid choice. Please enter C, Chat, or N.")
    ...
    print(f"--- Seeded Abilities ---")
    print(f"Recurrent Scratchpad: {config.use_scratchpad}, Internal Deliberator: {config.use_deliberator}, Confidence Circuit: {config.use_confidence_circuit}")

    # --- Data Loading and Main Training Loop ---
    if args.parquet_folder:
        if pd is None:
            raise ImportError("Pandas is not installed. Please run 'pip install pandas pyarrow' to train on parquet files.")
        ...
        for epoch_idx in range(start_epoch, len(parquet_files)):
            current_epoch_num = epoch_idx + 1
            parquet_file_path = parquet_files[epoch_idx]
            
            print(f"\n--- Starting Epoch {current_epoch_num}/{len(parquet_files)} (File: {os.path.basename(parquet_file_path)}) ---")
            try:
                df = pd.read_parquet(parquet_file_path)
                if args.text_column not in df.columns:
                    print(f"  [Warning] Text column '{args.text_column}' not found in {os.path.basename(parquet_file_path)}. Skipping file.")
                    continue
                text = ' '.join(df[args.text_column].astype(str).tolist())
                train_data = torch.tensor(tokenizer.encode(text))
                val_data = train_data # Use a small slice for validation metrics if needed
            except Exception as e:
                print(f"  [Error] Failed to read or process {os.path.basename(parquet_file_path)}: {e}. Skipping file.")
                continue

            epoch_start_time = time.time() # Start timer for ETA

            # Inner training loop for the current file
            possible_starts = list(range(len(train_data) - config.block_size - 1))
            random.shuffle(possible_starts)
            total_steps_in_epoch = len(possible_starts) // BATCH_SIZE
            for i in range(total_steps_in_epoch):
                # Training step logic (same as before)
                ...
                main_loss = ce_loss + (conf_loss * config.confidence_loss_weight if config.use_confidence_circuit else 0)
                ...
                if global_step % 100 == 0:
                    ...
                    steps_done_in_epoch = i + 1
                    steps_per_sec = steps_done_in_epoch / time_elapsed if time_elapsed > 0 else 0
                    steps_remaining = total_steps_in_epoch - steps_done_in_epoch
                    ...
                    progress = (steps_done_in_epoch / total_steps_in_epoch) * 100
                    
                    print(f"Epoch {current_epoch_num} [{progress:.1f}%] | Step {global_step} | ETA: {eta_str} | Loss {(main_loss + vq_optimizer_loss).item():.4f} (CE: {ce_loss.item():.4f}, Conf: {conf_loss.item():.4f}, Q: {q_loss.item():.4f})")
            ...
            # --- Save checkpoint after each file (epoch) ---
            checkpoint_path = os.path.join(CHECKPOINT_DIR, f"dmt_checkpoint_epoch_{current_epoch_num}.pth")
            print(f"Epoch complete. Saving checkpoint to {checkpoint_path}")
            torch.save({
    ...
    else: # --- Original training logic using input.txt ---
        print("Starting training on input.txt...")
        NUM_EPOCHS = 1
        ...
        for epoch in range(start_epoch, NUM_EPOCHS):
            ...
            print(f"\n--- Starting Epoch {current_epoch_num}/{NUM_EPOCHS} ---")
            epoch_start_time = time.time() # Start timer for ETA
            ...
            total_steps_in_epoch = len(possible_starts) // BATCH_SIZE
            for i in range(total_steps_in_epoch):
                # Training step logic (same as above)
                start_indices = possible_starts[i*BATCH_SIZE : (i+1)*BATCH_SIZE]
                xb = torch.stack([train_data[j:j+config.block_size] for j in start_indices]).to(device)
                yb = torch.stack([train_data[j+1:j+config.block_size+1] for j in start_indices]).to(device)
                logits, (ce_loss, q_loss, e_loss, conf_loss) = model(xb, yb)
